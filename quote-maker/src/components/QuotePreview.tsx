...
export const QuotePreview = ({ quote }: QuotePreviewProps) => {
    ...
    const tax = calculateTax(subtotal, quote.taxRate, quote.enableTax);
    ...
    // Reset scale when quote changes (content likely changed)
    useLayoutEffect(() => {
        setScale(1);
    }, [quote]);
            ...
            >
                ...
                >
                        ...
                        <div style={{ width: '55%', display: 'flex', flexDirection: 'column' }}>
                            ...
                            <div style={{ marginTop: 'auto' }}>
                                ...
                                <div style={{
                                    ...
                                    <span style={{ fontSize: '12pt', fontWeight: 'bold' }}>御見積金額</span>
                            ...
                            <div style={{ marginBottom: '20px', display: 'flex', flexDirection: 'column', alignItems: 'flex-end', fontSize: '10pt' }}>
                                {quote.showQuoteNumber && (
                                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '20px', marginBottom: '4px' }}>
                                        <span>見積番号</span>
                                        <span style={{ minWidth: '80px', textAlign: 'right' }}>{quote.quoteNumber}</span>
                                    </div>
                                )}
                                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '20px' }}>
                                    <span>見積日</span>
                                    <span style={{ minWidth: '80px', textAlign: 'right' }}>{formatDate(quote.date)}</span>
                                </div>
                            </div>
                            <h3 style={{ fontSize: '12pt', fontWeight: 'bold', textShadow: '0 0 1px #333', margin: 0 }}>{quote.fromName}</h3>
                            <p style={{ whiteSpace: 'pre-line', margin: 0, fontSize: '10pt' }}>{quote.fromAddress}</p>
                            {quote.fromPhone && <p style={{ margin: '0 0 4px 0', fontSize: '10pt' }}>TEL:{quote.fromPhone}</p>}
                            {quote.showInvoiceRegistrationNumber && quote.invoiceRegistrationNumber && (
                                <p style={{ margin: '4px 0 0 0', fontSize: '10pt' }}>登録番号: {quote.invoiceRegistrationNumber}</p>
                            )}
                        </div>
                    </div>
                    ...
                    <div style={{ fontSize: '10pt', lineHeight: 1.6, fontFamily: '"Roboto", "Noto Sans JP", sans-serif' }}>
                        <p style={{ textDecoration: 'underline', marginBottom: '40px', marginTop: '40px' }}>見積有効期限： {formatDate(quote.dueDate)}</p>
                    </div>

                    {/* Notes Box - Expanded */}
                    <div style={{ border: '1px solid #333', padding: '15px', minHeight: '90px', marginTop: '20px' }}>
                        <div style={{ display: 'flex', gap: '20px' }}>
                            <span style={{ fontWeight: 'normal', minWidth: '40px' }}>備考</span>
                            <span style={{ whiteSpace: 'pre-line' }}>{quote.notes}</span>
                        </div>
                    </div>
                </div>
            </div>
